...
contract BasedRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BASEDs to distribute in the pool.
        uint256 lastRewardTime; // Last time that BASEDs distribution occurred.
        uint256 accBasedPerShare; // Accumulated BASEDs per share, times 1e18. See below.
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "BasedRewardPool: existing pool?");
        }
    }
    ...
    // View function to see pending BASEDs on frontend.
    ...
    function safeBasedTransfer(address _to, uint256 _amount) internal {
        uint256 _basedBal = based.balanceOf(address(this));
        if (_basedBal > 0) {
            if (_amount > _basedBal) {
                based.safeTransfer(_to, _basedBal);
            } else {
